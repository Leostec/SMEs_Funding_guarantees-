...
def main(args):
    ...
    '''Run SFA'''
    # fit two-step models
    X_train_lime,key_list_lime,key_list= PEnTex_clf.fit(X_train, y_train)  #fit函数是定义在SFAClassifier里的函数，里面包括了SFA的整个流程，得到第一二阶段的值
    # predict
    PEnTex_clf.predict(X_train_lime,X_test, y_test,key_list_lime,key_list)  #predict函数是定义在SFAClassifier里的，作用是通过第一二阶段的模型，计算SFA性能

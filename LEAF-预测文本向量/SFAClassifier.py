...
#这两个方法分别用于定义实例子样本和特征子样本的下界和上界。它们根据数据集的大小来确定子样本的范围，以用于后续的模型训练
    ...
    def fit(self, X_train, y_train):
        ...
        X_train_ex_lime = X_train_data.join(train_df_lime)  # lime
        X_train_ex_lime = X_train_ex_lime.join(X_train_pca)
        X_train_ex_text=X_train_data.join(X_train_pca)
        # Train 3 second-stage models 在这三种增强特征上训练第二阶段的模型
        self.train_second_stage(X_train_ex_lime, y_train, 'base-lime-text',0)
    ...
    def predict(self, X_train, X_test, y_test, key_list_lime, key_list):
        ...
        test_lime, test_all_probas = self.predict_first_stage(X_train,X_test_data, y_test)   #使用第一阶段模型对测试集进行预测，得到原始预测结果 test_preds、Shapley 值 test_shap 和所有类别的概率预测 test_all_probas

        # use the OOP predictions and Shapley values to create 3 variations of augmented features
        test_df_lime = pd.DataFrame(test_lime, columns=[f'shap_{col}' for col in X_test_data.columns],
        ...
        X_test_ex_lime = X_test_ex_lime.join(X_test_pca)
        X_test_ex_text=X_test_data.join(X_test_pca)
        # predict using the second_stage model
        preds_lime = self.predict_second_stage(X_test_ex_lime, y_test, 'base-lime-text')   #使用第二阶段模型对使用 Shapley 值特征的测试集进行预测，得到预测结果 preds_shap
        ...
        test_all_probas=pd.DataFrame(data=test_all_probas)
        # test_all_probas.to_csv('D:/研究生/资产评估/基于大语言模型-生成式人工智能问答系统/preds_base.csv')
    ...
    def predict_second_stage(self, X_test, y_test, config):
        ...
        # preds.to_csv(f'D:/研究生/资产评估/基于大语言模型-生成式人工智能问答系统/preds_{config}.csv')
        # y_test.to_csv('D:/研究生/资产评估/基于大语言模型-生成式人工智能问答系统/y_test.csv')
        return preds

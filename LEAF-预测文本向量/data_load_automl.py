...
def datasets_to_X_y(ds_path):
    ...
    df1 = pd.read_csv('/Users/leo/研究生/资产评估/第二篇-基于大语言模型的评估系统/实验文件/文字embedding-全文本M3E.csv')  #读取数据集
    df2 = pd.read_csv('/Users/leo/研究生/资产评估/第二篇-基于大语言模型的评估系统/实验文件/400大类编号版.csv')

    # df = df.drop(columns=["报告编号"])
    # 初始化PCA模型
    # removed_column_data = df1["报告编号"]
    # X = df1.drop(columns=["报告编号"])
    # pca = PCA()
    # # 将数据拟合到PCA模型中
    # pca.fit(X)
    # # 获取每个主成分解释的方差百分比
    ...
    # # 将降维后的结果添加到原始DataFrame
    # df1 = pd.DataFrame(data=pca_result, columns=[f'PCA{i}' for i in range(1, n_components + 1)])
    # df1["报告编号"] = removed_column_data

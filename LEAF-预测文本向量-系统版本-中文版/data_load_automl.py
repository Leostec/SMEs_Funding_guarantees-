...
def datasets_to_X_y(ds_path):
    ...
    df1 = pd.read_csv('/Users/leo/研究生/资产评估/第二篇-基于大语言模型的评估系统/实验文件/文字embedding-发展战略+前景.csv')  # 读取数据集
    df2 = pd.read_csv('/Users/leo/研究生/资产评估/第二篇-基于大语言模型的评估系统/实验文件/400大类编号版.csv')
   # df = df.drop(columns=["报告编号"])
    # df = label_encode_class(df)  #标签换成二分类
    df1['报告编号'] = df1['报告编号'].astype(str)
    df2['报告编号'] = df2['报告编号'].astype(str)
    df = pd.merge(df2, df1, on='报告编号', how='inner')
    df = df.drop(columns=['报告编号'])
    y = df.iloc[:, 0]
    X = df.iloc[:, 1:45]
    x2 = df.iloc[:, 45:]
    x2.columns=[f'PCA{i}' for i in range(1, 2049)]
...
def read_all_data_files(all_pickle_path, file_pickle_path, files_path):
    ...
    ds_name, X, y,scaler1= datasets_to_X_y(data_file_path)   #使用 datasets_to_X_y 函数加载和预处理数据集，得到数据集的名称 ds_name、特征 X、目标列 y、分类特征信息 categorical 和经过独热编码的特征 X_oh（如果适用）
    ...
    return final_x, final_y, final_ds_details,scaler1   #返回指定的数据集特征、目标列和详细信息

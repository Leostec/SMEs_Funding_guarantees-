...
# 数据路径和文件名的常量
BINARY_DATA_PATH = '/Users/leo/研究生/资产评估/第二篇-基于大语言模型的评估系统/实验文件'
PICKLE_PATH = 'pickle'
BINARY_PICKLE_PATH = PICKLE_PATH + '/binary'
PICKLE_ALL_BINARY_DATASETS_PATH = PICKLE_PATH + '/binary_datasets_idx.pkl'
# test_path = 'D:/研究生/资产评估/论文撰写/Improved financing guarantee evaluation for SMEs by LIME-based feature augmentation/测试数据.csv'
def main(args,embeddings,method,test_path):
    # 解析命令行参数
    ...
    X, y, ds_details,scaler1 = read_all_data_files(all_datasets_path, pickle_path, data_path)
    ...
    '''Run SFA'''
    # fit three-step models

    if method == 'fit':
        X_train_lime,key_list_lime,key_list = PEnTex_clf.fit(X, y)  #fit函数是定义在SFAClassifier里的函数，里面包括了SFA的整个流程，得到第一二阶段的值
        with open('pickle/X_train_lime.pkl', 'wb') as f:
            pickle.dump(X_train_lime, f)
        with open('pickle/key_list_lime.pkl', 'wb') as f:
            pickle.dump(key_list_lime, f)
        with open('pickle/key_list.pkl', 'wb') as f:
            pickle.dump(key_list, f)
    if method == 'predict':
    # predict
        with open('pickle/X_train_lime.pkl', 'rb') as f:
            X_train_lime = pickle.load(f)
        with open('pickle/key_list_lime.pkl', 'rb') as f:
            key_list_lime = pickle.load(f)
        with open('pickle/key_list.pkl', 'rb') as f:
            key_list = pickle.load(f)
        X_test = pd.read_csv(test_path)
        column = X_test.columns.tolist()
    #     #标准化
        X_test=pd.DataFrame(X_test,columns=column)
        X_test=scaler1.transform(X_test)
        X_test= pd.DataFrame(X_test, columns=column)
        embedding=embeddings
        result=PEnTex_clf.predict(X_train_lime,X_test,embedding,key_list_lime,key_list)  #predict函数是定义在SFAClassifier里的，作用是通过第一二阶段的模型，计算SFA性能
        return result

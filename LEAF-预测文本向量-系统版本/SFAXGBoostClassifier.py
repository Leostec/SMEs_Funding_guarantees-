...
class SFAXGBoostClassifier(SFAClassifier):
    ...
    def predict_proba(self, clf, val_data):
        ...
        """
       #获取数据集
        if self.get_categories() is not None:   #若存在类别特征
            dvalid = DMatrix(val_data,enable_categorical=True)   #DMatrix 是 XGBoost 中的数据类型，用于存储和处理训练和验证数据。enable_categorical=True 表示启用类别特征，使得模型能够正确处理类别特征
        else:
            dvalid = DMatrix(val_data)   #若不存在则直接创建对象
        probs = clf.predict(dvalid)   #模型对每个类别的概率预测
        if self.get_n_classes() == 2:   #若是二分类问题
            probs = np.array([np.array([1-i, i]) for i in probs])   #将每个概率值 i 转换为 [1-i, i] 的格式，以满足 XGBoost 对二分类问题的要求
        return probs
    def predict_proba_train(self, clf, val_data):
        """
        Return the predicted probability for the given classifier.
        :param clf: XGBoost classifier
        :param val_data: data
        :return: val_data's predicted probability
        """
        x_val, y_val = val_data[0], val_data[1]   #获取数据集
        if self.get_categories() is not None:   #若存在类别特征
            dvalid = DMatrix(x_val, label=y_val, enable_categorical=True)   #DMatrix 是 XGBoost 中的数据类型，用于存储和处理训练和验证数据。enable_categorical=True 表示启用类别特征，使得模型能够正确处理类别特征
        else:
            dvalid = DMatrix(x_val, label=y_val)   #若不存在则直接创建对象
        probs = clf.predict(dvalid)   #模型对每个类别的概率预测
        if self.get_n_classes() == 2:   #若是二分类问题
            probs = np.array([np.array([1-i, i]) for i in probs])   #将每个概率值 i 转换为 [1-i, i] 的格式，以满足 XGBoost 对二分类问题的要求
        return probs

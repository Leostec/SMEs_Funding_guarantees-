...
#这两个方法分别用于定义实例子样本和特征子样本的下界和上界。它们根据数据集的大小来确定子样本的范围，以用于后续的模型训练
    ...
    def fit(self, X_train, y_train):
        ...
        val_preds, val_shap_values = self.train_first_stage(X_train_data, y_train)
        ...
        train_df_shap = pd.DataFrame(val_shap_values, columns=[f'shap_{col}' for col in X_train_data.columns],
        ...
        X_train_ex_p_shap = X_train_data.join(train_df_shap).join(train_df_preds)  # p-shap
        X_train_ex_shap = X_train_data.join(train_df_shap)  # shap
        X_train_ex_p = X_train_data.join(train_df_preds)  # p
        # join 文本
        X_train_ex_p_shap=X_train_ex_p_shap.join(X_train_pca)
        X_train_ex_shap=X_train_ex_shap.join(X_train_pca)
        X_train_ex_p=X_train_ex_p.join(X_train_pca)
        ...
        self.train_second_stage(X_train_pca, y_train, 'text')
        self.train_second_stage(X_train_ex_text, y_train, 'base-text')
    ...
    def predict(self, X_test, y_test):
        ...
        :return: SFA score
        """
        X_test_data=X_test.iloc[:,:44]
        X_test_pca=X_test.iloc[:,44:]
        # predict using the first-stage model
        test_preds, test_shap, test_all_probas = self.predict_first_stage(X_test_data, y_test)   #使用第一阶段模型对测试集进行预测，得到原始预测结果 test_preds、Shapley 值 test_shap 和所有类别的概率预测 test_all_probas
        ...
        test_df_shap = pd.DataFrame(test_shap, columns=[f'shap_{col}' for col in X_test_data.columns],
                                    index=X_test_data.index)

        X_test_ex_p_shap = X_test_data.join(test_df_shap).join(test_df_preds)  # p-shap
        X_test_ex_shap = X_test_data.join(test_df_shap)  # shap
        X_test_ex_p = X_test_data.join(test_df_preds)  # p

        X_test_ex_p_shap=X_test_ex_p_shap.join(X_test_pca)
        X_test_ex_shap=X_test_ex_shap.join(X_test_pca)
        X_test_ex_p=X_test_ex_p.join(X_test_pca)
        ...
        self.predict_second_stage(X_test_pca, y_test, 'text')
        self.predict_second_stage(X_test_ex_text, y_test, 'base-text')

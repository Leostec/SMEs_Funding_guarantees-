...
#这两个方法分别用于定义实例子样本和特征子样本的下界和上界。它们根据数据集的大小来确定子样本的范围，以用于后续的模型训练
    ...
    def fit(self, X_train, y_train):
        ...
        val_preds, val_lime_values , X_train_lime = self.train_first_stage(X_train, y_train)
        ...
        X_train_ex_p_lime = X_train.join(train_df_lime).join(train_df_preds)  # p-lime
    ...
    def train_first_stage(self, X_train_val, y_train_val):
        ...
        for i, (tr_ind, val_ind) in enumerate(kf.split(X_train_val, y_train_val)):
            ...
            if self.model_name in ['xgb', 'random_forest']:
                ...
                # # # lime值提取法一---提取i行
                # explanation = explainer.explain_instance(X_val.iloc[i].values, clf, num_features=44)
                # for j in range(44):
                #     # pdb.set_trace()
                #     idx = explanation.local_exp[1][j][0]
                #     val_lime_vals[i, idx] = explanation.local_exp[1][j][1]  # 存储每个样本的特征贡献值
                # # lime值提取法二---提取全部行
                # for x in range(len(X_val)):
                #     # pdb.set_trace()
                #     explanation = explainer.explain_instance(X_val.iloc[x].values, clf, num_features=44)
                #     x = x + N
                #     # 遍历每个样本，将特征贡献值赋值给对应位置
                ...
                # # 法2
                # clf=LGBMClassifier()
                # # my_lgbm._booster = clf
                # clf.fit(X_train, y_train)
                # clf=clf.predict_proba
                # explanation = explainer.explain_instance(X_val.iloc[i].values,predict_fn,num_features=len(y_val))
                # 法二
                # for x in range(len(X_val)):
                #     # pdb.set_trace()
                #     explanation = explainer.explain_instance(X_val.iloc[x].values, predict_fn, num_features=44)
                #     x = x + N
                #     # 遍历每个样本，将特征贡献值赋值给对应位置
                #     for j in range(X_train_val.shape[1]):
                #         # pdb.set_trace()
        ...
        return val_preds, val_lime_vals, X_train
    ...
    def predict(self, X_train, X_test, y_test, key_list_lime, key_list):
        ...
        test_preds, test_lime, test_all_probas = self.predict_first_stage(X_train,X_test, y_test)   #使用第一阶段模型对测试集进行预测，得到原始预测结果 test_preds、Shapley 值 test_shap 和所有类别的概率预测 test_all_probas
        ...
        X_test_ex_p_lime = X_test.join(test_df_lime).join(test_df_preds)  # p-lime
        ...
        preds_p_lime = self.predict_second_stage(X_test_ex_p_lime, y_test, 'p_lime')  #使用第二阶段模型对使用同时包含原始预测结果和 Shapley 值特征的测试集进行预测，得到预测结果 preds_p_shap
        ...
        total_score_mean = self.calc_average_test_score(test_all_probas, [preds_p,  #使用预测概率、三种预测结果和真实标签计算测试集的平均分数
                                                              preds_lime, preds_p_lime], y_test)
    ...
    def predict_first_stage(self,X_train, X_test, y_test):
        ...
        for i in range(self.n_folds):  #循环遍历每折
            ...
            if self.model_name in ['xgb', 'random_forest']:
                # my_xgb = XGBClassifier()
                # my_xgb._Booster = clf
                # # a=xgb_classifier.predict(X_train)
                # clf = my_xgb.predict_proba
                ...
                clf=self.load_model(path)
                # lime值提取法一---提取i行
                # explanation = explainer.explain_instance(X_test.iloc[i].values, clf, num_features=44)
                # for j in range(44):
                #     # pdb.set_trace()
                #     idx = explanation.local_exp[1][j][0]
                #     test_lime_vals[i,i, idx] = explanation.local_exp[1][j][1]  # 存储每个样本的特征贡献值
                # # lime值提取法二---提取全部行
                # for x in range(len(X_test)):
                #     # pdb.set_trace()
                #     explanation = explainer.explain_instance(X_test.iloc[x].values, clf, num_features=44)
                #     # 遍历每个样本，将特征贡献值赋值给对应位置
                ...
                # my_lgbm = LGBMClassifier()
                # my_lgbm._Booster = clf
                # clf = my_lgbm.predict_proba
                # explanation = explainer.explain_instance(X_test.iloc[i].values, predict_fn, num_features=len(y_test))
                # 法二
                # for x in range(len(X_test)):
                #     # pdb.set_trace()
                #     explanation = explainer.explain_instance(X_test.iloc[x].values, predict_fn, num_features=44)
                #     # 遍历每个样本，将特征贡献值赋值给对应位置
                #     for j in range(X_test.shape[1]):
                #         # pdb.set_trace()
                #         idx = explanation.local_exp[1][j][0]
            ...
            #     # pdb.set_trace()
            #     idx = explanation.local_exp[1][j][0]
            #     test_lime_vals[i, idx] = explanation.local_exp[1][j][1]  # 存储每个样本的特征贡献值
